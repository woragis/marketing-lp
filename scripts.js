...
function javascriptRender() {
  ...
  conteudoDiv.innerHTML = conteudo.map((tipo) => {
    let assuntos = tipo.conteudo.map((assunto) => "<li><a href=" + assunto.link + ">" + assunto.title + "</a></li>")
    return (
      "<li><h4>" + tipo.title + "</h4><ol>" + assuntos[0] + "</ol></li>"
    )
  })
}
